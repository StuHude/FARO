...
INPUT=${INPUT:-$ROOT/data/cyclegrpo_gres_20260727/gres_no_target_train_1k_seed42.jsonl}
...
JOB_PREFIX=${JOB_PREFIX:-dna-fepo-text-calibration}
...
[[ -n "${POSITIVE_TAGS_64}${POSITIVE_TAGS_16}" ]] || { echo "positive tags must be non-empty" >&2; exit 2; }
for gpu in "${GPU_LEVELS[@]}"; do
  job_name="${JOB_PREFIX}-${gpu}g-$(date +%s)"
  echo "SUBMIT gpu=${gpu} job=${job_name}"
  if (( gpu >= 8 )); then positive_tags="$POSITIVE_TAGS_64"; else positive_tags="$POSITIVE_TAGS_16"; fi
  submit_log=$(rjob submit --name="$job_name" --namespace=ailab-dnacoding \
    --cpu="$((gpu * 10))" --gpu="$gpu" --memory="$((gpu * 80000))" --positive-tags="$positive_tags" \
...
for i in \$(seq 0 $((gpu - 1))); do
  CUDA_VISIBLE_DEVICES=\$i /opt/vlm/bin/python -m projects.pixvl_idea3.eval.eval_text_calibration --config '$CONFIG' --adapter-path '$ADAPTER' --input '$INPUT' --task-id \$i --num-tasks '$gpu' --output \"\$TMP/part_\$i.json\" > \"\$TMP/log_\$i.txt\" 2>&1 &
done
wait
...
hits = sum(int(r['abstention'] == r['target_abstention']) for r in records)
Path(sys.argv[2]).write_text(json.dumps({'num_samples': len(records), 'abstention_accuracy': hits / max(len(records), 1), 'records': records}, ensure_ascii=False, indent=2))
PY
" 2>&1)
  printf '%s\n' "$submit_log"
