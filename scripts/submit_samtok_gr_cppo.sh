...
CONFIG=${CONFIG:-$FARO_ROOT/Sa2VA/projects/samtok_selective/configs/fepo_gr_cppo_one_step_2gpu.py}
ADAPTER=${SAMTOK_STANDALONE_ADAPTER:-$FARO_ROOT/outputs/samtok_selective/continued_sft_to500/adapter}
JOB_NAME=${JOB_NAME:-dna-samtok-fepo-gr-cppo-one-step-2g}
GPU_COUNT=${GPU_COUNT:-2}
# Sub-eight-GPU dnacoding jobs must use the registered partition tags.
TAGS_FILE=${TAGS_FILE:-$FARO_ROOT/rjob_tags.txt}

case "$JOB_NAME" in dna-*) ;; *) echo "JOB_NAME must start with dna-" >&2; exit 2 ;; esac
[[ "$GPU_COUNT" == "2" ]] || { echo "GR-CPPO requires GPU_COUNT=2" >&2; exit 2; }
...
python3 -m projects.samtok_selective.gr_cppo_contract \
