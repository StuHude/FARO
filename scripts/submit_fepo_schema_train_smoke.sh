...
cat >&2 <<'MSG'
Legacy PixVL routed-OPD trainer submission is disabled. Use a SAMTok-only
FEPO entry point with the registered 5,120-row/10-step contract.
MSG
exit 2
...
export FARO_WORKSPACE_ROOT='$ROOT'
...
mkdir -p '$ROOT/logs'; echo FEPO_SCHEMA_TRAIN_START | tee '$ROOT/logs/$JOB_NAME.log'
cd '$FARO_ROOT'
/opt/vlm/bin/torchrun --standalone --nproc_per_node='$GPU_COUNT' \
  -m projects.pixvl_idea3.trainers.joint_routed_opd_rl_trainer \
  --config '$CONFIG' \
  2>&1 | tee -a '$ROOT/logs/$JOB_NAME.log'
" 2>&1)
printf '%s\n' "$submit_log"
created_name=$(printf '%s\n' "$submit_log" | sed -n 's/.*created rjob_name: //p' | tail -1)
[[ -n "$created_name" ]] || {
  echo "rjob submission did not return a created rjob_name" >&2
  exit 1
}
