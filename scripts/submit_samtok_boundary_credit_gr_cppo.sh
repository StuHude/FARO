...
CONFIG=${CONFIG:?Set CONFIG to a boundary-credit config}
ADAPTER=${SAMTOK_STANDALONE_ADAPTER:-$FARO_ROOT/outputs/samtok_selective/continued_sft_to500/adapter}
JOB_NAME=${JOB_NAME:-dna-samtok-fepo-boundary-credit-2g}
...
case "$CONFIG" in *_one_step_*.py|*_one-step-*.py)
...
if [[ "$CONFIG" == *sign_balanced* ]]; then
  python3 -m projects.samtok_selective.sign_balanced_gr_cppo_contract \
    --repo-root "$FARO_ROOT" --adapter "$ADAPTER" --skip-model-hash
elif [[ "$CONFIG" == *greedy_relative* ]]; then
  python3 -m projects.samtok_selective.greedy_relative_gr_cppo_contract \
    --repo-root "$FARO_ROOT" --adapter "$ADAPTER" --skip-model-hash
elif [[ "$CONFIG" == *gain_preference* ]]; then
  python3 -m projects.samtok_selective.gain_preference_gr_cppo_contract \
    --repo-root "$FARO_ROOT" --adapter "$ADAPTER" --skip-model-hash
elif [[ "$CONFIG" == *greedy_preference* ]]; then
  python3 -m projects.samtok_selective.greedy_preference_gr_cppo_contract \
    --repo-root "$FARO_ROOT" --adapter "$ADAPTER" --skip-model-hash
elif [[ "$CONFIG" == *improvement_only* ]]; then
  python3 -m projects.samtok_selective.improvement_only_gr_cppo_contract \
    --repo-root "$FARO_ROOT" --adapter "$ADAPTER" --skip-model-hash
else
  python3 -m projects.samtok_selective.boundary_credit_gr_cppo_contract \
    --repo-root "$FARO_ROOT" --adapter "$ADAPTER" --skip-model-hash
fi
