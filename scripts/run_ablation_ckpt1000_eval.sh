...
OUT_ROOT=/mnt/pfs/xiaoyicheng/outputs/pixvl_idea3/ablation_ckpt1000_eval
mkdir -p "$OUT_ROOT"
...
Path("$out_dir/pred.json").write_text(
...
run_split_eval no_bucket 4 "$CFG_NO_BUCKET" "$ADAPTER_NO_BUCKET" > "$OUT_ROOT/split_no_bucket.log" 2>&1 &
P_SPLIT_NO_BUCKET=$!
run_split_eval shuffled 5 "$CFG_SHUFFLED" "$ADAPTER_SHUFFLED" > "$OUT_ROOT/split_shuffled.log" 2>&1 &
P_SPLIT_SHUFFLED=$!

run_official_dlc no_bucket 6 "$ADAPTER_NO_BUCKET" &
P_DLC_NO_BUCKET=$!
run_official_dlc shuffled 7 "$ADAPTER_SHUFFLED" &
P_DLC_SHUFFLED=$!

wait "$P_REF_NO_BUCKET"
wait "$P_REF_SHUFFLED"
wait "$P_SPLIT_NO_BUCKET"
wait "$P_SPLIT_SHUFFLED"
wait "$P_DLC_NO_BUCKET"
wait "$P_DLC_SHUFFLED"
