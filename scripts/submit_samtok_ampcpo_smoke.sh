...
CONFIG=${CONFIG:-$FARO_ROOT/Sa2VA/projects/samtok_selective/configs/fepo_ampcpo_smoke_2gpu.py}
ADAPTER=${SAMTOK_STANDALONE_ADAPTER:-$FARO_ROOT/outputs/samtok_selective/continued_sft/adapter}
JOB_NAME=${JOB_NAME:-dna-samtok-fepo-ampcpo-smoke-2g}
GPU_COUNT=${GPU_COUNT:-2}
TAGS_FILE=${TAGS_FILE:-$FARO_ROOT/rjob_tags.txt}

case "$JOB_NAME" in dna-*) ;; *) echo "JOB_NAME must start with dna-" >&2; exit 2 ;; esac
[[ "$GPU_COUNT" == "2" ]] || { echo "AM-CPPO smoke requires GPU_COUNT=2" >&2; exit 2; }
...
[[ ! -f "$MODEL/adapter_config.json" ]] || { echo "MODEL must be a base checkpoint" >&2; exit 2; }
...
python3 -m projects.samtok_selective.ampcpo_contract \
...
/opt/vlm/bin/torchrun --standalone --nproc_per_node=2 \
  -m projects.samtok_selective.fepo_ampcpo_trainer --config '$CONFIG' \
