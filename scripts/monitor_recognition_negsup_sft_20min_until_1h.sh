...
mkdir -p "$ROOT"
while true; do
  echo "[$(date '+%F %T %Z')] recognition_negsup_sft 20min check" >> "$LOG"
...
cmd="ps -eo etimes,cmd | rg 'accelerate launch --main_process_port .*idea3_recognition_negsup_sft_8gpu.py|joint_sft_trainer --config .*/idea3_recognition_negsup_sft_8gpu.py'"
