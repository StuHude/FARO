...
import inflect
import torch
from transformers import AutoModelForCausalLM, AutoProcessor, AutoTokenizer, Qwen2_5_VLForConditionalGeneration, Qwen3VLForConditionalGeneration
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--qa", default="/mnt/pfs/xiaoyicheng/BRIDGE-OPD/Sa2VA/godx7/DLC-Bench/qa.json")
    parser.add_argument("--class-names", dest="class_names", default="/mnt/pfs/xiaoyicheng/BRIDGE-OPD/Sa2VA/godx7/DLC-Bench/class_names.json")
    parser.add_argument("--judge-model", default="/mnt/pfs/xiaoyicheng/models/Qwen3-VL-4B-SAMTok-co")
...
def main() -> None:
    ...
    tokenizer = None
    processor = None
    if "Qwen3-VL" in judge_model_path:
        model = Qwen3VLForConditionalGeneration.from_pretrained(
            judge_model_path,
            torch_dtype="auto",
        ).to(args.device).eval()
        processor = AutoProcessor.from_pretrained(judge_model_path)
    elif "Qwen2.5-VL" in judge_model_path or "Qwen2_5-VL" in judge_model_path or "Qwen2.5-VL" in judge_model_path:
    ...
    def local_query(prompt: str, temperature: float, max_tokens: int, model_name: str) -> str:
        ...
        if tokenizer is not None:
            ...
            inputs = processor(text=[text], images=None, videos=None, return_tensors="pt").to(model.device)
    ...
    print(json.dumps({"avg_pos": avg_pos, "avg_neg": avg_neg, "avg": (avg_pos + avg_neg) / 2}))
