"""PES-FEPO: predicted-evidence scoped native-relative PPO credit."""

import copy
from pathlib import Path

from projects.samtok_selective.configs.tail_gppo_common import build_tail_gppo_config
from projects.samtok_selective.tail_gppo_contract import UNIFIED_PREDICTED_EVIDENCE_SCOPE_STAGE


config = copy.deepcopy(build_tail_gppo_config("plain_rank", 10))
config["stage"] = UNIFIED_PREDICTED_EVIDENCE_SCOPE_STAGE
...
config["checkpoint"]["output_dir"] = str(
    Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_PREDICTED_EVIDENCE_SCOPE_STAGE)
)
...
config["tail_gppo"].update(
    {
        ...
        "pareto_credit_mode": "predicted_evidence_scope",
        "depth_local_credit_policy": "native_reference_predicted_evidence_scope_first_divergence",
        "advantage": "native_reference_predicted_evidence_scope_mean_normalized",
        ...
        "pes_evidence_shuffle": False,
        "pes_evidence_shuffle_seed": 1907,
    }
)
