...
The RL objective remains the registered R18 native-relative rollout.  The
auxiliary term is ordinary supervised CE on a mildly photometric view of the
same row, with the unchanged ground-truth SAMTok answer as target.
"""
...
config["seed"] = 17
config["stage"] = UNIFIED_GROUNDED_INTERFACE_STAGE
...
config["checkpoint"]["output_dir"] = str(
    Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_GROUNDED_INTERFACE_STAGE)
)
