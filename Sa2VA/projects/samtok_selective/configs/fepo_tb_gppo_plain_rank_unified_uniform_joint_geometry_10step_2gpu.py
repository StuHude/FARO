"""R17: uniform joint cIoU/boundary positive credit, no frequency shaping."""

import copy
from pathlib import Path

from projects.samtok_selective.configs.tail_gppo_common import build_tail_gppo_config
from projects.samtok_selective.tail_gppo_contract import UNIFIED_UNIFORM_JOINT_STAGE


config = copy.deepcopy(build_tail_gppo_config("plain_rank", 10))
config["stage"] = UNIFIED_UNIFORM_JOINT_STAGE
...
config["checkpoint"]["output_dir"] = str(Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_UNIFORM_JOINT_STAGE))
...
config["tail_gppo"].update({
    ...
    "depth_local_credit_policy": "uniform_joint_geometry_no_rarity",
    "depth_local_decay": 1.0,
