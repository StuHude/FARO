"""R9: verified positive replay hybrid for standalone SAMTok GPPO.

The best sampled mask is replayed with a small on-policy token CE term only
when its raw cIoU beats native greedy decoding by the registered threshold.
"""
...
from projects.samtok_selective.tail_gppo_contract import UNIFIED_REPLAY_STAGE


config = copy.deepcopy(build_tail_gppo_config("plain_rank", 10))
config["stage"] = UNIFIED_REPLAY_STAGE
...
config["checkpoint"]["output_dir"] = str(
    Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_REPLAY_STAGE)
)
...
config["tail_gppo"].update(
    {
        ...
        "verified_replay_mode": "best_sampled_cIoU_replay",
