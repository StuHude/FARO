"""R32: persistent signed native-rank depth-local FEPO."""

import copy
from pathlib import Path

from projects.samtok_selective.configs.tail_gppo_common import build_tail_gppo_config
from projects.samtok_selective.tail_gppo_contract import UNIFIED_NATIVE_RANK_SIGNED_20_STAGE


config = copy.deepcopy(build_tail_gppo_config("plain_rank", 20))
config["seed"] = 17
config["stage"] = UNIFIED_NATIVE_RANK_SIGNED_20_STAGE
...
config["checkpoint"]["output_dir"] = str(
    Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_NATIVE_RANK_SIGNED_20_STAGE)
)
