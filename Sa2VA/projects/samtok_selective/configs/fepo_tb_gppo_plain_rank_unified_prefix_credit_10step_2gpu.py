"""R6: hierarchical geometry-prefix credit on top of the R5 raw-cIoU arm.

Only decoded-mask cIoU is used as reward.  Positive gains are allocated to
informative SAMTok code prefixes before the standard clipped GPPO update.
"""
...
from projects.samtok_selective.tail_gppo_contract import UNIFIED_PREFIX_STAGE


config = copy.deepcopy(build_tail_gppo_config("plain_rank", 10))
config["stage"] = UNIFIED_PREFIX_STAGE
...
config["checkpoint"]["output_dir"] = str(
    Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_PREFIX_STAGE)
)
...
config["tail_gppo"].update(
    {
        ...
        "advantage": "hierarchical_prefix_improvement_mean_normalized",
        "prefix_credit_mode": "hierarchical_geometry_prefix",
