"""R11: Pareto-verified, prefix-local replay for SAMTok geometry."""

import copy
from pathlib import Path

from projects.samtok_selective.configs.tail_gppo_common import build_tail_gppo_config
from projects.samtok_selective.tail_gppo_contract import UNIFIED_PARETO_PREFIX_REPLAY_STAGE


config = copy.deepcopy(build_tail_gppo_config("plain_rank", 10))
config["stage"] = UNIFIED_PARETO_PREFIX_REPLAY_STAGE
...
config["checkpoint"]["output_dir"] = str(
    Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_PARETO_PREFIX_REPLAY_STAGE)
)
...
config["tail_gppo"].update(
    {
        ...
        "verified_replay_mode": "best_sampled_pareto_prefix_replay",
