"""R12 compatibility screen: tie-aware group rank-Pareto geometry credit."""

import copy
from pathlib import Path

from projects.samtok_selective.configs.tail_gppo_common import build_tail_gppo_config
from projects.samtok_selective.tail_gppo_contract import UNIFIED_RANK_PARETO_STAGE


config = copy.deepcopy(build_tail_gppo_config("plain_rank", 10))
config["stage"] = UNIFIED_RANK_PARETO_STAGE
...
config["checkpoint"]["output_dir"] = str(
    Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_RANK_PARETO_STAGE)
)
...
config["tail_gppo"].update(
    {
        ...
        "pareto_credit_mode": "rank_pareto_geometry",
        "rank_pareto_tie_policy": "midrank_geomean_group_standardized",
    }
)
