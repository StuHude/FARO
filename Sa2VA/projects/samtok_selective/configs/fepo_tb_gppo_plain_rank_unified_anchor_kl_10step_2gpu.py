"""R24: native geometry FEPO with a frozen initialization anchor KL."""

import copy
from pathlib import Path

from projects.samtok_selective.configs.tail_gppo_common import build_tail_gppo_config
from projects.samtok_selective.tail_gppo_contract import UNIFIED_ANCHOR_KL_STAGE


config = copy.deepcopy(build_tail_gppo_config("plain_rank", 10))
config["seed"] = 17
config["stage"] = UNIFIED_ANCHOR_KL_STAGE
...
# Anchor-KL replays variable-length rows inside the same update as the policy
# graph.  Disable activation checkpointing for this isolated screen so PyTorch
# cannot reuse checkpoint metadata across the auxiliary KL forwards.  Other
# FEPO screens retain the memory-saving checkpoint configuration.
config["runtime"]["gradient_checkpointing"] = False
config["checkpoint"]["output_dir"] = str(
    Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_ANCHOR_KL_STAGE)
)
...
config["tail_gppo"].update(
    {
        ...
        "anchor_kl_enabled": True,
        "anchor_buffer_rows": 64,
        "anchor_kl_epsilon": 0.02,
        "anchor_kl_lambda": 0.5,
    }
)
