...
from projects.samtok_selective.tail_gppo_contract import UNIFIED_IMPROVEMENT_STAGE


config = copy.deepcopy(build_tail_gppo_config("plain_rank", 10))
config["stage"] = UNIFIED_IMPROVEMENT_STAGE
...
config["checkpoint"]["output_dir"] = str(
    Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_IMPROVEMENT_STAGE)
)
...
config["tail_gppo"].update(
    {
        ...
        "advantage": "positive_greedy_improvement_mean_normalized",
