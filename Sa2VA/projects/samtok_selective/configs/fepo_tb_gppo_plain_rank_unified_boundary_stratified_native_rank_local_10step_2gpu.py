"""BS-FEPO: fixed boundary-stratified sampling with frozen R18 credit."""

import copy
from pathlib import Path

from projects.samtok_selective.configs.tail_gppo_common import build_tail_gppo_config
from projects.samtok_selective.tail_gppo_contract import UNIFIED_BOUNDARY_STRATIFIED_STAGE


config = copy.deepcopy(build_tail_gppo_config("plain_rank", 10))
config["seed"] = 17
config["stage"] = UNIFIED_BOUNDARY_STRATIFIED_STAGE
...
config["checkpoint"]["output_dir"] = str(
    Path(config["checkpoint"]["output_dir"]).with_name(UNIFIED_BOUNDARY_STRATIFIED_STAGE)
)
...
config["tail_gppo"].update(
    {
        ...
        "advantage": "native_reference_boundary_stratified_mean_normalized",
        "pareto_credit_mode": "boundary_stratified_native_rank_local",
        "depth_local_credit_policy": "native_reference_boundary_stratum_rank_first_divergence",
        "depth_local_decay": 0.85,
        "boundary_stratified_schedule": True,
        "schedule": "registered_boundary_stratified_50_25_25",
        "boundary_sampling_mix": {
            "ordinary": 0.50,
            "thin": 0.25,
            "boundary_hard": 0.25,
        },
