from copy import deepcopy
from pathlib import Path
import runpy


base = runpy.run_path(Path(__file__).with_name("idea3_mvp_routed_opd_rl.py"))["config"]
config = deepcopy(base)

config["stage"] = "idea3_mvp_scale100k_3gpu_routed_opd_rl"
config["run_name"] = "idea3_mvp_scale100k_3gpu_routed_opd_rl"
config["checkpoint"]["output_dir"] = "/mnt/pfs/xiaoyicheng/outputs/pixvl_idea3/scale100k_3gpu_routed_opd_rl"

# 3 GPUs, keep per-GPU batch high but stable.
config["data"]["batch_size"] = 4
config["rl"]["group_size"] = {
    "refseg": 8,
    "maskcap": 4,
}

# 3 * 4 * 10000 = 120k samples seen.
config["optimizer"]["max_steps"] = 10000
config["optimizer"]["grad_accum_steps"] = 1
config["checkpoint"]["save_every"] = 500
config["logging"]["log_every"] = 10
config["logging"]["snapshot_every"] = 20
config["memory_optim"]["fsdp"]["enabled"] = False
