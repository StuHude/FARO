...
def parse_args():
    ...
    parser.add_argument(
        '--sam2_path',
        default="zhouyik/Qwen3-VL-4B-SAMTok-co/sam2.1_hiera_large.pt",
    ...
    parser.add_argument(
        '--image_folder',
        default='./data/glamm_data/images/coco2014/train2014/',
        help='Image root')
    parser.add_argument(
        '--temp_save_dir',
        default='./temp_save/refcoco',
        help='Temporary prediction save dir')
    ...
    parser.add_argument('--quiet-metric', action='store_true')
...
def metric():
    ...
    for json_file in os.listdir(args.temp_save_dir):
        with open(os.path.join(args.temp_save_dir, json_file), 'r') as f:
        ...
        if (not args.quiet_metric) and ciou < 0.7:
    ...
    vlm_eval_ap = []
    vlm_eval_ciou = []
    vlm_json_files = ['./data/PaDT-MLLM/RefCOCO/rec_jsons_processed/refcoco_val.json']
...
def main():
    ...
    if args.adapter_path:
        model = PeftModel.from_pretrained(model, args.adapter_path, is_trainable=False).cuda().eval()
    ...
    os.makedirs(args.temp_save_dir, exist_ok=True)
    ...
    for data_dict in tqdm.tqdm(all_data_dict[_start_:_end_]):
        ...
        if os.path.exists(f"{args.temp_save_dir}/{case_id}.json"):
            print("file exists.............")
            continue
        ...
        if len(quant_ids) == 0:
            ...
            with open(f"{args.temp_save_dir}/{case_id}.json", 'w') as f:
